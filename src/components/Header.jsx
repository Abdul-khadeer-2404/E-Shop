...
function Header() {
  ...
  return (
    <header className="bg-indigo-600 text-white">
              ...
              {['Home', 'Products', 'Cart', 'Login', 'Signup'].map((item) => (
                <li key={item}>
                  <Link
                    ...
                    className="relative py-2 transition duration-300 ease-in-out hover:text-indigo-200 group"
                  >
                    {item}
      ...
      {isMenuOpen && (
        <div className="lg:hidden">
          <nav className="px-4 pt-2 pb-4 space-y-2 bg-indigo-700">
            {['Home', 'Products', 'Cart', 'Login', 'Signup'].map((item) => (
              <Link
                key={item}
                to={item === 'Home' ? '/' : `/${item.toLowerCase()}`}
                className="block py-2 text-white hover:bg-indigo-600 rounded transition duration-300 ease-in-out"
